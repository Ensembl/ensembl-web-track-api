...
def test_startup_checksum_is_reused(setup_cache, monkeypatch):
    ...
    with monkeypatch.context() as patch:
        patch.setattr(
            Path, "open", Mock(side_effect=AssertionError("must not read again"))
        )
        cache.initialize_database_version()
        for _ in range(3):
            assert (
                cache.get_database_version() == hashlib.sha256(b"initial").hexdigest()
            )


def test_identical_copies_have_same_checksum(
    setup_cache, settings, tmp_path, monkeypatch
):
...
def test_missing_database_at_startup_bypasses_cache(
    stateful_cache, isolated_redis, monkeypatch
):
    ...
    cache.initialize_database_version()
    handler = Mock(return_value=Response({}))
    response = cache.redis_cache("test")(handler)(
        None, Request(APIRequestFactory().get("/"))
    )
    assert response["X-Track-Cache"] == "BYPASS"
    assert "X-Track-Cache-Write" not in response
    handler.assert_called_once()
...
def test_explicit_ttl_override(stateful_cache, isolated_redis):
    handler = Mock(return_value=Response({}))
    cache.redis_cache("test", ttl=60)(handler)(
        None, Request(APIRequestFactory().get("/"))
    )
...
def cached_track(db, stateful_cache):
    ...
    DatasetRelease.objects.create(
        genome_id=genome_id, dataset_id=dataset_id, release_label="2026-01-01"
    )
...
def test_endpoint_hits_skip_database(cached_track, endpoint, django_assert_num_queries):
    ...
    client = APIClient()
    identifier = (
        cached_track.genome_id
        if endpoint == "track_categories"
        else cached_track.track_id
    )
