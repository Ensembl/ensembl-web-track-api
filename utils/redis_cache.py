...
def initialize_database_version():
    ...
    try:
        ...
        logger.warning(
            "Cannot hash database; bypassing cache until restart", exc_info=True
        )
        ...
        def wrapper(self, request, *args, **kwargs):
            if not settings.ENABLE_CACHE:
                return _mark_cache_response(
                    func(self, request, *args, **kwargs), "BYPASS"
                )
            ...
            try:
                ...
                logger.warning(
                    "Redis cache read failed; bypassing cache for this request",
                    exc_info=True,
                )
            except (ValueError, UnicodeError):
                outcome = "ERROR"
                logger.warning(
                    "Invalid cached response; querying database", exc_info=True
                )
            ...
            if cache_write_allowed and response.status_code == 200:
                try:
                    redis_client.setex(
                        key,
                        settings.CACHE_TTL if ttl is None else ttl,
