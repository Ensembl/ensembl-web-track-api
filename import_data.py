...
if(len(sys.argv) > 1):
    if('.' in sys.argv[1]):
        ...
        print("Data importer for track API endpoint.\nUsage:", sys.argv[0], ' <input_file.yaml>')
...
with open(yaml_file) as f:
    ...
    try:
        ...
        for genome_id in data.keys():
            ...
            genome_obj, created = Genome.objects.get_or_create(genome_id = genome_id) #get() => [create() => save()]
            if(not created):
                sys.exit("Genome ID '%s' already imported!\nEmpty the database before running data import: ./manage.py flush" % genome_id)
            categories = data[genome_id]
            for category in categories:
                category_obj = Category.objects.create(label = category['label'], track_category_id = category['track_category_id'], genome = genome_obj)
                for type in category['types']:
                    category_type_obj, created = CategoryType.objects.get_or_create(type = type) #reuse stored category types
                    category_obj.types.add(category_type_obj) #link Category => CategoryType
                for track in category['track_list']:
                    track_obj = Track.objects.create(label = track['label'], track_id = track['track_id'], category = category_obj)
